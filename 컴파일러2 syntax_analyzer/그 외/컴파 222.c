...
char reduceLS[33][7] = {
"S", "CODE","CODE","CODE","VDECL","VDECL",	
"ASSIGN","FDECL","ARG","MOREA","BLOCK",
"STMT","STMT","STMT","STMT","STMT","ELSE",
"RHS","RHS","EXPR","EXPR","TERM","TERM",
...
//특정한 양만큼 원소 삭제 및 배열 크기를 줄이는 함수
void reduceSize(int index, int reduceAmount) {
    for(int i = index; i < size; i++)
        strcpy(tokenArray[i], tokenArray[i + reduceAmount]);
    
    size = size - reduceAmount;
    
}

int shiftDecision(int* splitterIndex, char* currState)
{
    for(int i = 0; i < 97; i++)
    {
        if(!strcmp(currState,shift[i][0]))
        {
            printf("shift1");
            if(!strcmp(tokenArray[*splitterIndex],shift[i][1]))
            {
                strcpy(currState, shift[i][2]);
                push(currState);
                (*splitterIndex)++;
                printf("shift2\n");
                return 1;
...
int reduceDecision(int splitterIndex, char* currState)
{
    for(int i = 0; i < 104; i++)
    {
        if(!strcmp(currState,reduce[i][0]))
            if(!strcmp(tokenArray[splitterIndex],reduce[i][1]))
            {
                n = atoi(reduce[i][2]); //contex free grammar 정보
                if(n == 0)
                    accept = 1;
                strcpy(tokenArray[splitterIndex],reduceLS[n]);
                reduceSize(splitterIndex+1, reduceRS[n]-1);
                
                for(int j = 0; j < reduceRS[n]; j++)    //step1
                    pop();
                strcpy(currState, stack[top]);
                
                
                for(int i = 0; i < 52; i++)     //step2
                {
                    if(!strcmp(currState,GOTO[i][0]))
                        if(!strcmp(reduceLS[n],GOTO[i][1]))
                        {
                            strcpy(currState, GOTO[i][2]);
                            push(currState);
                            printf("reduce2");
                            return 1;
                        }    
...
int main(int argc, char *argv[])
{
    ...
    while(accept == 0)
    {
        printf("%d %s\n", splitterIndex, currState);
        if(shiftDecision(&splitterIndex, currState) == 0)
            if(reduceDecision(splitterIndex, currState) == 0)
            {
                //print_error(argv[1]);
                return 0;
            }
